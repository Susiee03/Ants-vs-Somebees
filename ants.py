...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        """
        self.prospective_base_place = self.place
        self.distance = 0
        while True:
            if self.prospective_base_place.bees == []:
                if self.prospective_base_place.entrance == beehive:
                    return None
                else:
                    self.prospective_base_place = self.prospective_base_place.entrance
                    self.distance += 1
            else:
                if self.min_range <= self.distance <= self.max_range:
                    return rANTdom_else_none(self.prospective_base_place.bees)
                else:
                    if self.prospective_base_place.entrance == beehive:
                        return None
                    else:
        ...
        if self.armor > amount:
            ...
            if self.armor <= 0:
                if len(self.place.bees) > 0:
                    for bee in self.place.bees[:]:
                        bee.reduce_armor(self.damage)   # reduce_armor, if armor = 0, remove bee from place and send callback
            self.place.remove_insect(self)
            self.death_callback()
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        unluckly_bee = rANTdom_else_none(bee)
        if unluckly_bee is not None:
            unluckly_bee.reduce_armor(unluckly_bee.armor)
...
class WallAnt(Ant):
    """WallAnt provides protection and cannot be removed by the player."""
